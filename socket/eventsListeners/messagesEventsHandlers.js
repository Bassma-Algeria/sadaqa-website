  ...
  (dispatch, getState) => {
    ...
    if (userCurrentlyInMessagesPage(contactsList)) {
      if (!messageSenderExistInContactsList(contactsList, message.sender_id)) {
        ...
        messageSenderConversationOpen(message.sender_id, chatParticipant)
      ) {
        ...
        dispatch(
          setLatestMessage({
            targetContactId: message.sender_id,
            newMessage: message,
          })
        );
      }
...
const messageSenderConversationOpen = (
  messageSenderId,
  currentChatParticipant
) => {
  return (
    currentChatParticipant && messageSenderId === currentChatParticipant.user_id
  );
};
