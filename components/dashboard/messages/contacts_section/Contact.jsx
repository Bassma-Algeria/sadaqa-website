...
const Contact = ({
  ...
  const [isContactConversationOpen, setIsContactConversationOpen] =
    useState(false);
  ...
  return (
    <div
      className={`${styles.contactContainer} ${
        hasUnreadMessges(latestMessage, currentUserId) &&
        styles.hasUnreadMessages
      } ${isContactConversationOpen && styles.contactConversationOpen}`}
      onClick={() => router.push(`?chatParticipantId=${contactId}`)}
    >
      <div className={styles.profilePicContainer}>
        <img src={getProfilePic(contactProfilePic?.link, gender)} alt="" />
