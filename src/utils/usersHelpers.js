...
const getDefaultProfilePic = gender => {
  if (gender === 'male' || gender === '') {
    return defaultProfilePicMale;
  } else {
    return defaultProfilePicFemale;
  }
};
...
const getWilayaId = wilayaName => {
  ...
  const targetWilaya = wilayas.find(wilaya => wilaya.name.toLowerCase().includes(wilayaToFound));
...
const isTypesNotAllowed = (role, type) => {
  return role !== 'association' && (type === 'call_for_help' || type === 'family_in_need');
};
