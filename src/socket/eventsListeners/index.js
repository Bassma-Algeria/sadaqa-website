...
import {
  newLikeNotification,
  newPostNotification,
} from "./notificationsEventsHandlers";
...
const registerSocketEventsListeners = (dispatch) => {
  ...
  socket.on(SET_ONLINE_USERS, (...args) =>
    dispatch(setOnlineUsersHandler(args))
  );
  ...
  socket.on(MAKE_MESSAGES_READ, (...args) =>
    dispatch(makeMessagesReadSocketHandler(args))
  );
  ...
  socket.on(NEW_LIKE_NOTIFICATION, (...args) =>
    dispatch(newLikeNotification(args))
  );

  socket.on(NEW_POST_NOTIFICATION, (...args) =>
    dispatch(newPostNotification(args))
  );
};
