...
const posts = createSlice({
  ...
  reducers: {
    ...
    inactivePostInAuthUserPosts: (state, {payload}) => {
      state.authUserPosts.data.find(post => post.post_id === payload).active = false;
    },

    deletePostInAuthUserPosts: (state, {payload}) => {
      state.authUserPosts.data = state.authUserPosts.data.filter(post => post.post_id !== payload);
    },
...
function handleLikePost(state, {payload}) {
  const {postId, isPostLiked, type, subType, likesCount, isSearchedPost, isAuthUserPost} = payload;

  if (isSearchedPost) {
    if (isPostLiked) {
      state.searchedPosts.data.find(post => post.post_id === postId).likes_count = likesCount - 1;
    } else {
      state.searchedPosts.data.find(post => post.post_id === postId).likes_count = likesCount + 1;
    }
  } else if (isAuthUserPost) {
    if (isPostLiked) {
      state.authUserPosts.data.find(post => post.post_id === postId).likes_count = likesCount - 1;
    } else {
      state.authUserPosts.data.find(post => post.post_id === postId).likes_count = likesCount + 1;
    }
  } else if (type === 'donation') {
    if (isPostLiked) {
      state.donation[subType].data.find(post => post.post_id === postId).likes_count =
        likesCount - 1;
    } else {
      state.donation[subType].data.find(post => post.post_id === postId).likes_count =
        likesCount + 1;
    ...
    if (isPostLiked) {
      state.peopleNeedHelp[subType].data.find(post => post.post_id === postId).likes_count =
        likesCount - 1;
    } else {
      state.peopleNeedHelp[subType].data.find(post => post.post_id === postId).likes_count =
        likesCount + 1;
