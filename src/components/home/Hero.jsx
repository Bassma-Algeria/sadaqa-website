    ...
    <HeroContainer>
      <Slider {...heroSliderSettings} className="home_slider_container">
        {heroData.map((element, index) => {
          return <HeroContent key={index} {...element} setIsPopupOpen={setIsPopupOpen} />;
        })}
      </Slider>
    </HeroContainer>
  );
...
const HeroContent = ({
  ...
  return (
    <div className={styles.hero} style={{ flexDirection: reverse && 'row-reverse' }}>
      <div className={styles.content} style={{ padding: reverse ? '0' : '0 0 0 100px' }}>
        <h1>{title}</h1>
        <p>{desc}</p>
      ...
      <div className={styles.image}>
        ...
        >
          <Image placeholder="blur" src={imageUrl} alt={imageAlt} />
        </div>
      </div>
    </div>
  );
