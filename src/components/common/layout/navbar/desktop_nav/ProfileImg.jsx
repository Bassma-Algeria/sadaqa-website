...
import { getProfilePic, getNameToShow } from '../../../../../utils/usersHelpers';
...
const ProfileImg = () => {
  ...
  return (
    <div className={styles.profileImgWithDashboardLinks} id="dashboadLinksContainer">
      <div className={styles.profileImgContainer}>
        <ProfileImageContainer isOpen={isDashboardLinksOpen} setIsOpen={setIsDashboardLinksOpen} />
      </div>

      {isDashboardLinksOpen && (
        <DashboardLinks isOpen={isDashboardLinksOpen} setIsOpen={setIsDashboardLinksOpen} />
      )}
    </div>
  );
};
...
const DashboardLinks = ({ isOpen, setIsOpen }) => {
  ...
  useEffect(() => {
    const handleClick = e => {
      if (!isClickedElementInsideTarget('dashboadLinksContainer', e.target)) setIsOpen(false);
    };
  ...
  return (
    <div className={styles.dashboardLinks}>
      <div className={styles.header}>
        <ProfileImageContainer isOpen={isOpen} setIsOpen={setIsOpen} withoutNumOfMessages />
      ...
      {dashboardLinks.map((link, index) => {
        return (
          <LinkToShow link={link} key={index} index={index} closeLinks={() => setIsOpen(false)} />
        );
      })}
    </div>
  );
