...
// helpers
import { getPostThumbnail, linkFormatToTextFormat } from '../../../utils/postsHelpers';
...
const AdCard = props => {
  return (
    <div
      ref={props.adCardRef}
      className={`${styles.adCard} ${props.isGrid ? styles.grid : styles.inline} ${
    ...
    >
      ...
      <div className={styles.details}>
        ...
        <div className={styles.tag}>
          <Link href={`/people_need_help/${props.type}`}>{linkFormatToTextFormat(props.type)}</Link>
        </div>

        <Link href={`/posts/${props.post_id}`}>
          <h1>{props.title.length >= 17 ? props.title.slice(0, 17) + '..' : props.title}</h1>
        </Link>
